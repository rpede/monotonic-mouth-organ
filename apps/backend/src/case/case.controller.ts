...
import { getReportLength, withCharacterCount } from "../report-utils";
...
export class CaseController {
  ...
  async getStatus(@Param("caseNo") caseNo: string) {
    return await this.db.case.findUnique({ where: { caseNo }, include: { company: true } });
  }

  @Get(':caseNo/length')
  async getLength(@Param("company") companyName: string, @Param("caseNo") caseNo: string) {
    return getReportLength(companyName, caseNo);
  }
